...
export function initTaskRunLogsTable(db: any): void {
  db.exec(`
    ...
    `);
  db.exec(
    `CREATE INDEX IF NOT EXISTS idx_task_logs ON task_run_logs(task_id, executed_at)`,
  );
}
...
export function updateTaskAfterRun(
  ...
  db.prepare(
    `
        ...
        WHERE id = ?
    `,
  ).run(nextRun ?? 0, now, lastResult, finalStatus, now, taskId);
}
...
export function cleanupOldMessages(
  ...
  // Get all session IDs
  const sessions = db.prepare('SELECT id FROM sessions').all() as {
    id: string;
  }[];
  let deletedCount = 0;
...
export function getDatabaseStats(): {
  ...
  const scheduledTasks = (
    db.prepare('SELECT COUNT(*) as count FROM scheduled_tasks').get() as {
      count: number;
    }
  ).count;
  const taskRunLogs = (
    db.prepare('SELECT COUNT(*) as count FROM task_run_logs').get() as {
      count: number;
    }
  ).count;
