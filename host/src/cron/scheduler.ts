...
/**
 * Tool result display callback
 */
export interface ToolResultDisplayCallback {
    (toolCallId: string, result: string, subtype?: string): void;
}
...
export interface CronServiceOptions {
    ...
    onToolResult?: ToolResultDisplayCallback;
}
...
export class CronService {
    ...
    private readonly onToolResult?: ToolResultDisplayCallback;
    ...
    private async pushResultToChannel(
        ...
        if (pushed) {
            ...
            console.log(`[CronService] Failed to push task ${task.id} result to ${task.channelType}`);
        }
    }
...
export const cronService = new CronService({ pollIntervalMs: 60000 }); // 每分钟检查一次
