...
function toLarkDomain(
  domain: BotCredentials['domain'],
): Lark.Domain | undefined {
...
export async function fetchBotInfo(creds: BotCredentials): Promise<BotInfo> {
  ...
  try {
    ...
    const res = await client.im.chat
      .get({ path: { chat_id: '' } })
      .catch(() => null);
    ...
    if (botRes.code !== 0) {
      ...
      const bot = (botRes.bot ??
        (botRes.data as Record<string, unknown>)?.bot) as
